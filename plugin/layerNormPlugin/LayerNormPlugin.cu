...
namespace
{
char const* kLAYER_NORM_PLUGIN_NAME{"MyLayerNorm"};
    ...
    void const* const* inputs, void* const* outputs, void* workspace, cudaStream_t stream) noexcept
{
    ...
    case DataType::kFLOAT:
    {
        ...
        status = computeLayerNorm<float>(gridSize, nHiddenSize, input, output, gamma, beta, mEpsilon, stream);
        break;
    }
    case DataType::kHALF:
    {
        ...
        status = computeLayerNorm<half>(gridSize, nHiddenSize, input, output, gamma, beta, mEpsilon, stream);
        break;
    }
    // case DataType::kINT8:
    // {
